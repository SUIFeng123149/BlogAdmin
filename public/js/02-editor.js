  ...
  async function syncSectionCategoriesFromConfig() { try { const r=await api("/api/data/sections"); const bySlug={}; (r.items||[]).forEach(sec=>{ bySlug[sec.slug]=[...(sec.categories||[])]; }); postCategoriesBySection=bySlug; } catch(e){} }
    ...
    function refreshPostCategoryOptions(preferred="") {
      ...
      postCategoryField.value=categories.includes(current)?current:(current===""?"":(categories[0]||""));
    }
    ...
    function refreshMarkdownPreview() { const preview=$("#markdown-preview"); if(!window.marked) { preview.textContent="正在加载 Markdown 预览..."; return; } window.marked.setOptions({ breaks:true, gfm:true }); preview.innerHTML=window.marked.parse(postForm.elements.body.value||""); const slug=articleStorageKey(); if(slug) preview.querySelectorAll("img[src],video[src],video source[src]").forEach(media=>{ const raw=media.getAttribute("src")||""; if((raw.startsWith("./")&&raw.includes(".assets/"))||raw.includes("/post-assets/"))
			media.setAttribute("src",`/api/posts/${encodeURIComponent(slug)}/assets/${encodeURIComponent(raw.split("/").pop())}`);
			if(media.tagName==="IMG") media.onclick=()=>openImageLightbox(media.getAttribute("src")||"", media.alt||"图片预览"); }); addPreviewHeadingAnchors(preview); if(!preview.innerHTML.trim()) preview.innerHTML='<p class="muted">开始写作即可看到预览。</p>'; }
    ...
    function newPost() { showToast("新建文章","填写标题和内容后保存即可。"); currentSlug=null; postForm.reset(); refreshArticleCover(); mountPostTags([]); refreshPostCategoryOptions(); if(typeof syncPostSectionSelect==="function") syncPostSectionSelect("",""); refreshMarkdownPreview(); postForm.elements.published.value=new Date().toISOString().slice(0,10); $("#delete-post").classList.add("hidden"); show("editor"); }
    async function edit(slug) { const data=await api("/api/posts/"+encodeURIComponent(slug)); currentSlug=slug; fill(postForm,data); refreshPostCategoryOptions(data.category); if(typeof syncPostSectionSelect==="function") syncPostSectionSelect(data.contentSection||"",data.category||""); refreshMarkdownPreview(); $("#delete-post").classList.remove("hidden"); show("editor"); }
    ...
    // 登录前 #app 处于 hidden，同步分类表会 401：等待登录完成后重试（与 01 分区加载保持一致）
    function retrySyncSectionCategories(attempt) { const app=document.getElementById("app"); if(!app||!app.classList.contains("hidden")) { syncSectionCategoriesFromConfig(); return; } if((attempt||0)>=60) return; setTimeout(()=>retrySyncSectionCategories((attempt||0)+1),300); }
    retrySyncSectionCategories();
