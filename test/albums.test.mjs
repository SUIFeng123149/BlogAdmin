...
test("provides an album workspace in the admin interface", async () => {
	...
	const scripts = await readFile(new URL("../public/js/03-albums.js", import.meta.url), "utf8");
	assert.match(scripts, /albumNav\.dataset\.view="albums"/);
	assert.match(page, /id="albums"/);
	assert.match(scripts, /function loadAlbums\(/);
});
